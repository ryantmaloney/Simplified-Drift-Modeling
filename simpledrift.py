...
def driftmodeling(flynum, numberofbins, numberofdays, prefmean, prefvariance, envimean, envivariance, driftvariance, gain, per, maxsurvivalrate, birthrate, matureage, percentbh):
    ...
    if prefvariance>=0.015/percentbh: # Gaussian when prefvariance is more than 0.015
        pref[:,0,0,0]=sci.norm.pdf(x,prefmean,prefvariance) # A fly's first day preference gaussian of preference with center around 0
    ...
    if prefvariance>=0.015:
        pref[:,0,0,1]=sci.norm.pdf(x,prefmean,np.multiply(prefvariance,percentbh)) # A gaussian of preference with center around 0
    else:
